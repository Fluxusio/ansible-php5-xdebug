...
HEADER='memTotalMB   memFreeMB   memUsedMB  memFreePct  memUsedPct   pgPageOut  swapUsedPct   pgSwapOut   cSwitches  interrupts       forks   processes     threads  loadAvg1mi    cpuUser  cpuSystem    cpuIdle    cpuWait'
HEADERIZE="BEGIN {print \"$HEADER\"}"
PRINTF='END {printf "%10d  %10d  %10d  %10.1f  %10.1f  %10s   %10.1f  %10s  %10s  %10s  %10s  %10s  %10s  %10.2f %10d %10d %10d %10d\n", memTotalMB, memFreeMB, memUsedMB, memFreePct, memUsedPct, pgPageOut, swapUsedPct, pgSwapOut, cSwitches, interrupts, forks, processes, threads, loadAvg1mi, cpuUser, cpuSystem, cpuIdle, cpuWait}'
DERIVE='END {memUsedMB=memTotalMB-memFreeMB-memBuffMB-memCacheMB; memUsedPct=(100.0*memUsedMB)/memTotalMB; memFreePct=100.0-memUsedPct; swapUsedPct=swapUsed ? (100.0*swapUsed)/(swapUsed+swapFree) : 0}'

if [ "x$KERNEL" = "xLinux" ] ; then
  assertHaveCommand uptime
  assertHaveCommand ps
  assertHaveCommand vmstat
  CMD='eval uptime ; ps -e | wc -l ; ps -eT | wc -l ; vmstat | grep -Pe "^\s*\d+" ; vmstat -s'
  PARSE_0='NR==1 {loadAvg1mi=0+$(NF-2)} NR==2 {processes=$1} NR==3 {threads=$1} NR==4 {cpuUser=$13} NR==4 {cpuSystem=$14} NR==4 {cpuIdle=$15} NR==4 {cpuWait=$16}'
